...
public class RecursoBean implements Serializable {
	...
	public String saveAction(ActionEvent event) {
		...
		if (validateSaveAction(event)) {
			try {
				...
				FacesUtils.addMessageError(event, "Error al guardar el recurso", e.getMessage());
	...
	private boolean validateSaveAction(ActionEvent event) {
		...
		} if (StringUtils.isBlank(this.recurso.getNombre())) {
			FacesUtils.addMessageError(event,"Se debe ingresar el nombre del recurso", "");
			valid = false;

		} if (this.recurso.getTipoRecurso() == null || this.recurso.getTipoRecurso().getId() == null) {// TODO Validar el objeto tipo recurso
			FacesUtils.addMessageError(event, "Se debe ingresar el tipo del recurso", "");
			valid = false;
		}

		if (!valid) {
			FacesUtils.addMessageError(event, "Error al guardar el Recurso", detail);
	...
	public String deleteAction(ActionEvent event) {
		...
		try {
			...
			FacesUtils.addMessageError(event, "Error al eliminar el Recurso", e.getMessage());
